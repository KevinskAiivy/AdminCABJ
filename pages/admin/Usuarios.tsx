...
export const Usuarios = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-20 px-4 animate-boca-entrance">
        ...
        {isModalOpen && (
            <div className="fixed inset-0 z-[1000] flex justify-center p-0 bg-[#001d4a]/50 backdrop-blur-sm animate-in fade-in duration-300" style={{ paddingTop: 'calc(7rem - 4rem)', paddingBottom: '0', alignItems: 'flex-start' }}>
                <div className="relative w-full bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col border border-white/60 max-w-2xl animate-in zoom-in-95 duration-300 max-h-[calc(100vh-7rem+4rem)]">
                    <div className="liquid-glass-dark p-4 text-white flex items-center justify-between shrink-0">
                        <div className="flex items-center gap-3">
                            <div className="bg-white/10 p-2 rounded-xl"><User size={20} className="text-[#FCB131]" /></div>
                            <div>
                                <h2 className="oswald text-lg font-black uppercase tracking-tight">{editingUser ? 'Editar Usuario' : 'Nuevo Usuario'}</h2>
                                <p className="text-white/50 text-[9px] font-bold uppercase tracking-widest">Gestión de Acceso</p>
                            </div>
                        </div>
                        <X onClick={() => setIsModalOpen(false)} className="cursor-pointer opacity-60 hover:opacity-100 p-1.5 hover:bg-white/10 rounded-full transition-colors" size={20} />
                    </div>

                    <div className="p-5 space-y-5 overflow-y-auto custom-scrollbar flex-1 bg-gradient-to-b from-white to-gray-50/50">
                        {error && (
                            <div className="flex items-center gap-2 bg-red-50 text-red-600 p-3 rounded-lg border border-red-200 text-[10px] font-bold animate-in fade-in slide-in-from-top-2 shadow-sm">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <div className="relative group">
                                        ...
                                        />
                                        <Mail size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#003B94] transition-colors" />
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div className="space-y-1">
                                    ...
                                    <div className="relative group">
                                        ...
                                        />
                                        <Fingerprint size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#003B94] transition-colors" />
                                    </div>
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[8px] font-black text-gray-500 uppercase tracking-widest ml-1 flex items-center gap-1">
                                    ...
                                    <div className="relative group">
                                        ...
                                        />
                                        <KeyRound size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#003B94] transition-colors" />
                                    </div>
                                </div>
                            </div>
                        </div>

                        {/* Section: Permisos y Asignación */}
                        <div className="space-y-3 pt-3 border-t border-gray-200">
                            <h3 className="text-[#003B94] font-black uppercase text-[10px] tracking-widest border-b border-[#003B94]/20 pb-1.5 flex items-center gap-1.5">
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                    {[
                                        ...
                                        <button 
                                            ...
                                            className={`relative py-2.5 px-2 rounded-lg text-[7px] font-black uppercase tracking-widest border transition-all ${
                                                formData.role === role.value 
                                                    ? `${role.color} border-current shadow-md` 
                                                    : 'bg-white text-gray-400 border-gray-200 hover:border-gray-300 hover:bg-gray-50'
                                            }`}
                                        >
                                            <div className="font-black text-[9px] mb-0.5">{role.label}</div>
                                            <div className="text-[6px] opacity-70">{role.desc}</div>
                                            {formData.role === role.value && (
                                                <div className="absolute -top-1 -right-1 w-3 h-3 bg-[#FCB131] rounded-full flex items-center justify-center">
                                                    <Check size={8} className="text-[#001d4a]" />
                                                </div>
                                            )}
                                        </button>
                                    ))}
                            ...
                            {(formData.role === 'PRESIDENTE' || formData.role === 'REFERENTE') && (
                                <div className="space-y-1.5 animate-in fade-in slide-in-from-top-2 bg-blue-50/50 p-3 rounded-lg border border-blue-100">
                                    ...
                                    <div className="relative group">
                                        ...
                                        <Building2 size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#003B94] transition-colors pointer-events-none" />
                                        <ChevronDown size={14} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" />
                                    </div>
                                </div>
                            )}

                            <div className="flex items-center justify-between bg-gradient-to-r from-gray-50 to-white p-3 rounded-lg border border-gray-200 hover:border-[#003B94]/30 transition-all">
                                <div className="flex items-center gap-2.5">
                                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center transition-all ${formData.active ? 'bg-emerald-100 text-emerald-600' : 'bg-gray-100 text-gray-400'}`}>
                                        {formData.active ? <Check size={16} /> : <X size={16} />}
                                    </div>
                                    <div>
                                        <label className="text-[9px] font-black uppercase tracking-widest text-[#001d4a] cursor-pointer">
                                            Usuario Activo
                                        </label>
                                ...
                                <label className="relative inline-flex items-center cursor-pointer">
                                    ...
                                    />
                                    <div className="w-10 h-5 bg-gray-200 peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-[#003B94]/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-[#003B94]"></div>
                                </label>
                            </div>
                        </div>
                    </div>

                    <div className="p-4 bg-[#FCB131] border-t border-[#e5a02d] flex justify-end gap-2 shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] z-20 relative">
                        <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 rounded-lg font-black uppercase text-[8px] tracking-widest text-[#001d4a] bg-white border border-white/50 hover:bg-white/90 transition-colors shadow-sm">Cancelar</button>
