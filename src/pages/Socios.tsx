...
export const Socios = ({ user }: { user?: any }) => {
  ...
  // Reset position when modals open
  useEffect(() => {
      if (isEditModalOpen || isDeleteModalOpen || isExportModalOpen || showSaveConfirm) {
          setPosition({ x: 0, y: 0 });
      }
  }, [isEditModalOpen, isDeleteModalOpen, isExportModalOpen, showSaveConfirm]);
  ...
  const getGenderLabel = (label: string, gender: string = 'M') => {
      if (label === 'Socio') return gender === 'F' ? 'Socia' : 'Socio';
      if (label === 'Nacido') return gender === 'F' ? 'Nacida' : 'Nacido';
      return label;
  };
  ...
  }, [socios, searchQuery, filterStatus, filterCategory, filterConsulado, filterRole]);
  ...
  const resetFilters = () => {
      ...
      setFilterConsulado('ALL'); setFilterRole('ALL'); setCurrentPage(1);
  };
  ...
  const handleEdit = (socio: Socio) => {
    ...
    setFormData({ ...socio, lastMonthPaid: paidDate });
  ...
  const executeSave = async () => {
    ...
    const finalData = {
        ...formData, id: finalId, status: finalStatus.label as any,
        consulado: pendingConsulado !== null ? pendingConsulado : formData.consulado,
        role: finalRole, lastMonthPaid: dbPaymentDate, name: `${formData.firstName} ${formData.lastName}`
    };
    ...
    try {
        if (selectedSocio) {
            if (pendingIdChange) {
                ...
                await dataService.addSocio(finalData as Socio);
            } else {
                await dataService.updateSocio(finalData as Socio);
            }
        } else {
            await dataService.addSocio(finalData as Socio);
        }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-20 px-4 animate-boca-entrance">
        ...
        {currentItems.map((socio) => {
            ...
            <GlassCard key={socio.id} onClick={() => setViewSocio(socio)} className={`flex flex-col group relative overflow-hidden transition-all duration-300 hover:-translate-y-1 ${containerClass} p-0`}>
                <div className="p-5 pb-3 flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-2 items-center">
                            <span className="px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest border bg-gray-100 text-gray-500 border-gray-200">{socio.role || 'SOCIO'}</span>
                ...
                <div className="mt-auto px-5 py-3 border-t border-black/5 bg-white/50 flex justify-between items-center">
                    <span className="text-[9px] font-bold uppercase tracking-widest text-gray-400">ID: #{socio.id}</span>
      ...
      {/* EXPORT MODAL - CENTERED FIXED & DRAGGABLE */}
      {isExportModalOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 pointer-events-none">
            <div 
                style={{ transform: `translate(${position.x}px, ${position.y}px)`, transition: isDragging ? 'none' : 'transform 0.2s' }}
                className="pointer-events-auto relative w-full max-w-lg bg-white rounded-[2rem] shadow-2xl overflow-hidden flex flex-col border border-white/60 animate-in fade-in zoom-in-95 duration-300"
            >
                <div 
                    onMouseDown={handleDragStart}
                    className="bg-[#003B94] p-6 text-white flex items-center justify-between shrink-0 cursor-move select-none active:cursor-grabbing"
                >
                    <div className="flex items-center gap-4">
                        <div className="p-3 bg-white/10 rounded-xl"><FileText size={24} className="text-[#FCB131]" /></div>
                        <div><h2 className="oswald text-xl font-black uppercase tracking-tight">Exportar Padrón</h2><p className="text-white/50 text-[10px] font-bold uppercase tracking-widest">Generar PDF Oficial</p></div>
                    </div>
                    <X onClick={() => setIsExportModalOpen(false)} className="cursor-pointer opacity-60 hover:opacity-100 p-2 hover:bg-white/10 rounded-full transition-colors" size={24} />
                </div>
                <div className="p-6 flex-1 overflow-y-auto custom-scrollbar max-h-[60vh]">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-[#001d4a] font-black uppercase text-xs tracking-widest">Seleccionar Consulados</h3>
                        <button onClick={() => setExportSelection(exportSelection.has('ALL') ? new Set() : new Set(['ALL']))} className="text-[9px] font-bold text-[#003B94] hover:underline uppercase">{exportSelection.has('ALL') ? 'Deseleccionar Todos' : 'Seleccionar Todos'}</button>
                    </div>
                    <div className="space-y-2">
                        <div onClick={() => { const newSet = new Set(exportSelection); if (newSet.has('ALL')) newSet.clear(); else newSet.add('ALL'); setExportSelection(newSet); }} className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${exportSelection.has('ALL') ? 'bg-[#003B94] border-[#003B94] text-white shadow-md' : 'bg-gray-50 border-gray-200 text-gray-500 hover:bg-gray-100'}`}>
                            <span className="text-xs font-bold uppercase tracking-wide">Todos los Consulados</span>
                            {exportSelection.has('ALL') && <Check size={14} strokeWidth={3} />}
                        </div>
                        {!exportSelection.has('ALL') && consulados.map(c => (
                            <div key={c.id} onClick={() => { const newSet = new Set(exportSelection); if (newSet.has(c.name)) newSet.delete(c.name); else newSet.add(c.name); setExportSelection(newSet); }} className={`flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${exportSelection.has(c.name) ? 'bg-[#003B94] border-[#003B94] text-white shadow-md' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
                                <span className="text-xs font-bold uppercase tracking-wide">{c.name}</span>
                                {exportSelection.has(c.name) && <Check size={14} strokeWidth={3} />}
                            </div>
                        ))}
                    </div>
                </div>
                <div className="p-6 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
                    <button onClick={() => setIsExportModalOpen(false)} className="px-6 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest text-gray-500 bg-white border border-gray-200 hover:bg-gray-100">Cancelar</button>
                    <button onClick={handleExportPDF} className="bg-[#003B94] text-white px-8 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest shadow-xl flex items-center gap-2 hover:bg-[#001d4a] transition-all"><Printer size={16} /> Generar PDF</button>
                </div>
            </div>
        </div>
      )}
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 pointer-events-none">
             ...
             >
                <div 
                    onMouseDown={handleDragStart}
                    className="relative bg-gradient-to-r from-[#003B94] to-[#001d4a] p-6 text-white shrink-0 overflow-hidden cursor-move select-none active:cursor-grabbing"
                >
                    <div className="flex items-center gap-5 relative z-10">
                        <div className="w-16 h-16 rounded-2xl bg-white/10 border border-white/20 shadow-inner flex items-center justify-center text-2xl font-black text-[#FCB131]">
                            {formData.firstName ? `${formData.firstName[0]}${formData.lastName ? formData.lastName[0] : ''}` : <User size={32}/>}
                        </div>
                        <div>
                            <h2 className="oswald text-3xl font-black uppercase tracking-tight leading-none mb-1">{selectedSocio ? `Editar ${getGenderLabel('Socio', formData.gender)}` : `Nuevo ${getGenderLabel('Socio', formData.gender)}`}</h2>
                            <div className="flex items-center gap-3"><p className="text-[#FCB131] text-[10px] font-bold uppercase tracking-widest">{selectedSocio ? `ID: #${selectedSocio.id}` : 'Alta en Proceso'}</p></div>
                        </div>
                    </div>
                    <X onClick={() => setIsEditModalOpen(false)} className="cursor-pointer opacity-60 hover:opacity-100 p-2 hover:bg-white/10 rounded-full transition-colors absolute top-6 right-6 z-20" size={32} />
                </div>

                <div className="p-8 space-y-8 overflow-y-auto custom-scrollbar flex-1 bg-white/50">
                    <div className="space-y-4">
                        <h3 className="text-[#003B94] font-black uppercase text-[11px] tracking-widest border-b border-[#003B94]/10 pb-2 flex items-center gap-2"><User size={14}/> {getGenderLabel('Datos del Socio', formData.gender)}</h3>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="space-y-1"><label className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Apellido</label><input type="text" className="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 font-bold text-xs outline-none focus:bg-white focus:border-[#003B94]/30 uppercase text-[#001d4a]" value={formData.lastName} onChange={e => setFormData({...formData, lastName: e.target.value.toUpperCase()})}/></div>
                            <div className="space-y-1"><label className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Nombre</label><input type="text" className="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 font-bold text-xs outline-none focus:bg-white focus:border-[#003B94]/30 capitalize text-[#001d4a]" value={formData.firstName} onChange={e => setFormData({...formData, firstName: e.target.value})}/></div>
                            <div className="space-y-1"><label className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Género</label><select className="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 font-bold text-xs outline-none focus:bg-white focus:border-[#003B94]/30 text-[#001d4a]" value={formData.gender} onChange={e => setFormData({...formData, gender: e.target.value as any})}><option value="M">Masculino</option><option value="F">Femenino</option><option value="X">X (No binario)</option></select></div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="space-y-1"><label className="text-[9px] font-black text-gray-400 uppercase tracking-widest">DNI / Pasaporte</label><input type="text" className="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 font-bold text-xs outline-none focus:bg-white focus:border-[#003B94]/30 text-[#001d4a]" value={formData.dni} onChange={e => setFormData({...formData, dni: e.target.value})}/></div>
                            <div className="space-y-1"><label className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Último Pago</label><input type="text" className="w-full bg-white border border-gray-200 rounded-xl py-3 px-4 font-bold text-xs outline-none text-[#001d4a] tracking-widest" placeholder="DD/MM/AAAA" value={formData.lastMonthPaid || ''} onChange={e => setFormData({...formData, lastMonthPaid: e.target.value})}/></div>
                            <div className="space-y-1"><label className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Categoría</label><select className="w-full bg-gray-50 border border-gray-200 rounded-xl py-3 px-4 font-bold text-xs outline-none focus:bg-white focus:border-[#003B94]/30 text-[#001d4a]" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value as any})}>{SOCIO_CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-[#001d4a] p-6 rounded-2xl border border-[#FCB131]/30 shadow-lg relative overflow-hidden">
                ...
                <div className="p-6 bg-[#FCB131] border-t border-[#e5a02d] flex justify-end gap-4 shrink-0 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] z-20 relative">
                    <button onClick={() => setIsEditModalOpen(false)} className="px-6 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest text-[#001d4a] bg-white border border-white/50 hover:bg-white/90 transition-colors shadow-sm">Cancelar</button>
                    <button onClick={() => setShowSaveConfirm(true)} className="bg-[#003B94] text-white px-8 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest shadow-xl flex items-center gap-2 hover:bg-[#001d4a] transition-all"><Save size={16} /> Guardar</button>
                </div>
             </div>
        </div>
      )}

      {/* Save Confirmation Modal - CENTERED FIXED & DRAGGABLE */}
      {showSaveConfirm && (
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4 pointer-events-none">
            <div 
                style={{ transform: `translate(${position.x}px, ${position.y}px)`, transition: isDragging ? 'none' : 'transform 0.2s' }}
                className="pointer-events-auto bg-white p-8 rounded-2xl shadow-2xl max-w-sm text-center animate-in zoom-in-95 border border-gray-100"
            >
                <div onMouseDown={handleDragStart} className="cursor-move absolute inset-0 h-12 z-10" />
                <div className="w-16 h-16 bg-[#FCB131]/10 text-[#FCB131] rounded-full flex items-center justify-center mx-auto mb-4 relative z-0"><Save size={32}/></div>
                <h3 className="oswald text-xl font-bold text-[#001d4a] mb-2 relative z-0">¿Confirmar cambios?</h3>
      ...
      {isDeleteModalOpen && selectedSocio && (
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4 pointer-events-none">
            <div 
                style={{ transform: `translate(${position.x}px, ${position.y}px)`, transition: isDragging ? 'none' : 'transform 0.2s' }}
                className="pointer-events-auto bg-white p-8 rounded-2xl shadow-2xl max-w-sm text-center animate-in zoom-in-95 border border-red-100"
            >
                <div onMouseDown={handleDragStart} className="cursor-move absolute inset-0 h-12 z-10" />
                <div className="w-16 h-16 bg-red-50 text-red-500 rounded-full flex items-center justify-center mx-auto mb-4 relative z-0"><AlertTriangle size={32}/></div>
                <h3 className="oswald text-xl font-bold text-red-600 mb-2 relative z-0">Eliminar Socio</h3>
                <p className="text-sm text-gray-500 relative z-0">¿Estás seguro de eliminar a <strong>{selectedSocio.name}</strong>?</p>
                <div className="flex gap-3 mt-6 relative z-0">
